...
#            'pipelines.UpdatePricePipeline':1200,
    ...
    # def get_starturls():
    #     response_page = requests.get('http://www.mimocook.com/pt/')
    #     sel_page = Selector(response_page)
    #     categorylink = sel_page.xpath('//div[@class="cbp-category-link-w"]/a/@href').extract()
    #     categorylink.append('http://www.mimocook.com/pt/promocoes')
    #     return categorylink
    #
    # start_urls = get_starturls()
    #start_urls = ['file:///home/sukai/product.xml',]
    start_urls = ['file:///home/sukai/product.json',]
    ...
    #     for row in root:
    #         print row.find("post_title").text
    def parse(self,reponse):
        item = ProductItem()
        data = json.load(response.body)
        for d in data:
            item['name'] = d['post_title']
            print item['name']
